...
def _get_client_id(conn, first_name = None, last_name = None, email = None, phone_number = None):
    with conn.cursor() as cur:    
        if first_name and last_name:
            ...
            cur.execute("""SELECT client_id
                             FROM phones
                            WHERE phone_number = '{}';""".format(phone_number))
        else:
            print('Недостаточно информации для поиска. Необходимо ввести имя и фамилию, либо email, либо номер телефона!')
            sys.exit(1)
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""INSERT INTO clients(first_name, last_name, email) 
                        VALUES('{}', '{}', '{}') 
                        RETURNING client_id;""".format(first_name, last_name, email))
...
def add_phone_number(conn, phone_number, first_name = None, last_name = None, email = None, client_id=None):
    try:
        if not client_id:
            client_id = _get_client_id(conn, first_name=first_name, last_name=last_name, email=email)
        with conn.cursor() as cur:
            cur.execute("""INSERT INTO phones(phone_number, client_id) 
                        VALUES({}, {});""".format(phone_number, client_id))
            conn.commit()
    except psycopg2.errors.UniqueViolation:
...
def delete_phone_number(conn, first_name = None, last_name = None, email = None, client_id=None):
    if not client_id:
        client_id = _get_client_id(conn, first_name=first_name, last_name=last_name, email=email)
    with conn.cursor() as cur:
...
def find_client_id(conn, first_name = None, last_name = None, email = None, phone_number = None):
    client_id = _get_client_id(conn, first_name, last_name, email, phone_number)
    print("Id клиента:", client_id)


# Изменение данных клиента
def change_client_info(conn, first_name = None, last_name = None, email = None, phone_number = None, new_first_name = None, new_last_name = None, new_email = None):
    client_id = _get_client_id(conn, first_name=first_name, last_name=last_name, email=email, phone_number=phone_number)
    with conn.cursor() as cur:
        if new_first_name:
            cur.execute("""UPDATE clients SET first_name = '{}' WHERE client_id = {};""".format(new_first_name, client_id))
        if new_last_name:
            cur.execute("""UPDATE clients SET last_name = '{}' WHERE client_id = {};""".format(new_last_name, client_id)) 
        if new_email:
            cur.execute("""UPDATE clients SET email = '{}' WHERE client_id = {};""".format(new_email, client_id))      
        conn.commit()  
    print("Данные успешно изменены")  


with psycopg2.connect(database='netology_db', user='admin', password='admin',host='localhost',port='5432') as conn:
    create_db(conn)
